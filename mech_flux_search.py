...
# smiles1 = 'CCCCO[O]'
# smiles2 = 'C[CH]CCOO'  # 4744

# smiles1 = 'C[CH]CC'
# smiles2 = 'CC=CC'   # 278

smiles1 = 'C[CH]CCOO'
smiles2 = 'CC1OCC1'   # 449 <--- not found in the mechanism
...
ref_sp2 = rmgpy.species.Species(smiles=smiles2)

ref_index1 = database_fun.get_unique_species_index(ref_sp1)
ref_index2 = database_fun.get_unique_species_index(ref_sp2)
print(f"Looking for reactions involving species indices: {ref_index1} ({smiles1}), {ref_index2} ({smiles2})")
...
for j, rxn in enumerate(reaction_list1):
    if np.any([ref_sp1.is_isomorphic(rxn.reactants[i]) for i in range(len(rxn.reactants))]) and \
       np.any([ref_sp2.is_isomorphic(rxn.products[i]) for i in range(len(rxn.products))]):
        matching_reaction_indices.append(j)
    elif np.any([ref_sp2.is_isomorphic(rxn.reactants[i]) for i in range(len(rxn.reactants))]) and \
